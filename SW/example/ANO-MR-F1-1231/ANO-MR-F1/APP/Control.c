...
{
	T_float_angle angle;
	angle.rol = att_in->rol - (rc_in->ROLL-1500)/5*0.35f;
	angle.pit = att_in->pit + (rc_in->PITCH-1500)/5*0.35f;
	...
	PID_ROL.dout = PID_ROL.D * (gyr_in->X-rc_in->ROLL_SPEED/5.0f*0.35f);
	...
	PID_PIT.dout = PID_PIT.D * (gyr_in->Y+rc_in->PITCH_SPEED/5.0f*0.35f);
	PID_PIT.iout = PID_PIT.I * pit_i;
	if(rc_in->YAW<1480||rc_in->YAW>1520)
	{gyr_in->Z=gyr_in->Z+(rc_in->YAW-1500)*20.0f;}//*PID_PID_2.P
